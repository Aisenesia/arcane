...
UserData parseUserData(const std::string &data)
{
    ...
    if (data.find("user1:") != std::string::npos && data.find("user2:") != std::string::npos) {
        ...
        if (user1End == std::string::npos) user1End = data.length();
        ...
        if (tokenPos != std::string::npos && charPos != std::string::npos) {
            ...
            if (tokenEnd == std::string::npos) tokenEnd = user1Data.length();
            
            size_t charStart = charPos + 10; // length of "character:"
            size_t charEnd = user1Data.find(";", charStart);
            if (charEnd == std::string::npos) charEnd = user1Data.length();
...
UserData parseUser2Data(const std::string &data)
{
    ...
    if (data.find("user1:") != std::string::npos && data.find("user2:") != std::string::npos) {
        ...
        if (tokenPos != std::string::npos && charPos != std::string::npos) {
            ...
            if (tokenEnd == std::string::npos) tokenEnd = user2Data.length();
            
            size_t charStart = charPos + 10; // length of "character:"
            size_t charEnd = user2Data.find(";", charStart);
            if (charEnd == std::string::npos) charEnd = user2Data.length();
...
bool checkCardsInReadyArea(const cv::Mat &frame)
{
    ...
    for (int i = 0; i < arr.size; i++)
    {
        ...
        if (arr.results[i].classId > 1) // Cards have classId > 1, dice has classId = 1
        {
...
bool checkDiceVisible(const cv::Mat &frame)
{
    ...
    for (int i = 0; i < arr.size; i++)
    {
        if (arr.results[i].classId == 1) // Dice class ID
        {
            return true;
        }
    }
...
void processUnrealCommand(cv::VideoCapture& cap)
{
            ...
            bool user1Valid = !user1Data.token.empty() && !user1Data.characterId.empty();
              if (user1Valid) {
                ...
                if (apiResponses[0].success)
                {
                    ...
                    std::cout << "Entry Token: " << apiResponses[0].roomCode << std::endl;
    ...
    while (true)
    {
        ...
        int cardCount = 0;
        int diceCount = 0;
        std::vector<int> diceIndices;
        
        // Count cards and dice, store dice indices for classification
        for (int i = 0; i < detectionResults.size; i++)
        {
            if (detectionResults.results[i].classId == 0) // Dice
            {
                diceCount++;
                diceIndices.push_back(i);
            }
            else if (detectionResults.results[i].classId > 0) // Cards (class 2 and above)
            {
                cardCount++;
            }
        }
        
        // Output card detection results
        if (cardCount > 0)
        {
            std::cout << "CARDS DETECTED: " << cardCount << std::endl;
            for (int i = 0; i < detectionResults.size; i++)
            {
                if (detectionResults.results[i].classId > 1) // Cards
                {
                    cv::Rect box = detectionResults.results[i].boundingBox;
                    float confidence = detectionResults.results[i].confidence;
                    std::cout << "Card - ClassID: " << detectionResults.results[i].classId 
                             << ", Position: (" << box.x << "," << box.y << "," << box.width << "," << box.height << ")"
                             << ", Confidence: " << confidence << std::endl;
                }
            }
        }
          // Process dice detection and classification
        if (diceCount > 0)
        {
            std::cout << "DICE DETECTED: " << diceCount << std::endl;
            for (int diceIdx : diceIndices)
            {
                cv::Rect diceBox = detectionResults.results[diceIdx].boundingBox;
                float detectionConfidence = detectionResults.results[diceIdx].confidence;
                
                // Crop dice region for classification
                cv::Mat diceCrop = frame(diceBox);
                
                // Classify dice face
                ClassificationResult classResult = NetworkManager::classify(diceCrop);
                
                std::cout << "Dice - Position: (" << diceBox.x << "," << diceBox.y << "," << diceBox.width << "," << diceBox.height << ")"
                         << ", Detection Confidence: " << detectionConfidence
                         << ", Face Classification: " << classResult.classId
                         << ", Classification Confidence: " << classResult.confidence << std::endl;
            }
        }
          // Set visibility flags based on detection results
        bool cardsVisible = (cardCount > 0);
        bool diceVisible = (diceCount > 0);
        
        // Draw detection results on frame
        for (int i = 0; i < detectionResults.size; i++)
        {
            cv::Rect box = detectionResults.results[i].boundingBox;
            int classId = detectionResults.results[i].classId;
            float confidence = detectionResults.results[i].confidence;
            ...
            if (classId == 1) // Dice
            {
                color = cv::Scalar(0, 255, 0); // Green for dice
                objectType = "DICE";
            }
            else // Cards (class 2 and above)
            {
                color = cv::Scalar(255, 0, 0); // Red for cards
                objectType = "CARD";
            }
            ...
            std::string label = objectType + " (C:" + std::to_string(confidence).substr(0, 4) + ")";
...
{
    std::cout << "\n" << std::string(60, '=') << std::endl;
    std::cout << "           READY COMMAND - TESTING MODE" << std::endl;
    std::cout << std::string(60, '=') << std::endl;

    // Initialize networks
    if (!NetworkManager::initializeNetworks("dtc.onnx", "cls.onnx", true))
    {
        std::cerr << "❌ Failed to initialize networks." << std::endl;
        return;
    }
...
{
    std::cout << "\n" << std::string(60, '=') << std::endl;
    std::cout << "           DICE COMMAND - TESTING MODE" << std::endl;
    std::cout << std::string(60, '=') << std::endl;
    ...
    while (true)
    {
        ...
        for (int i = 0; i < arr.size; i++)
        {
            cv::Rect box = arr.results[i].boundingBox;
            int classId = arr.results[i].classId;
            float confidence = arr.results[i].confidence;

            cv::Scalar color = (classId == 1) ? cv::Scalar(0, 255, 0) : cv::Scalar(255, 0, 0); // Green for dice, red for cards
            cv::rectangle(frame, box, color, 2);
            
            std::string label = "ID:" + std::to_string(classId) + " (" + std::to_string(confidence).substr(0, 4) + ")";
            cv::putText(frame, label, cv::Point(box.x, box.y - 10), cv::FONT_HERSHEY_SIMPLEX, 0.5, color, 2);
        }

        bool diceVisible = checkDiceVisible(frame);
        std::string diceStatus = diceVisible ? "DICE: VISIBLE" : "DICE: NOT VISIBLE";
        cv::putText(frame, diceStatus, cv::Point(30, 30), cv::FONT_HERSHEY_SIMPLEX, 0.7,
            diceVisible ? cv::Scalar(0, 255, 0) : cv::Scalar(0, 0, 255), 2);
...
{
    std::cout << "\n" << std::string(60, '=') << std::endl;
    std::cout << "           CALIBRATE COMMAND - TESTING MODE" << std::endl;
    std::cout << std::string(60, '=') << std::endl;

    // Setup calibration checker
    cv::Point centers[] = { cv::Point(150, 150), cv::Point(1130, 570) };
    int radii[2] = { 60, 60 };
